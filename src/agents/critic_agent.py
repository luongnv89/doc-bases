    ...
    def __init__(self, llm):
        ...
        self.critique_prompt = ChatPromptTemplate.from_messages(
            [
                (
                    "system",
                    """You are a critical evaluator. Assess answers for:
    ...
    async def critique(self, question: str, answer: str, documents: str) -> Critique:
        ...
        try:
            result = await chain.ainvoke(
                {"question": question, "documents": documents, "answer": answer, "format_instructions": self.parser.get_format_instructions()}
            )

            logger.info(
                f"Critique scores: acc={result.accuracy_score:.2f}, " f"comp={result.completeness_score:.2f}, " f"clarity={result.clarity_score:.2f}"
            )
            return result
