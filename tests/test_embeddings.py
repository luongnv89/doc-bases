...
def test_get_embedding_model_openai(mock_embeddings):
    ...
    embeddings = get_embedding_model(provider="openai", model=TEST_MODEL, api_key=TEST_API_KEY, api_base=TEST_API_BASE)
    assert isinstance(embeddings, Embeddings)
...
def test_get_embedding_model_missing_api_key(mock_env_vars):
    ...
    with patch.dict(os.environ, {"OPENAI_API_KEY": ""}):
        with pytest.raises(ValueError, match="OPENAI_API_KEY not found in environment variables or user input."):
