...
def mock_llm():
    ...
    with patch("langchain_google_genai.ChatGoogleGenerativeAI") as mock_google, patch(
        "langchain_openai.ChatOpenAI"
    ) as mock_openai, patch("langchain_groq.ChatGroq") as mock_groq, patch(
...
def test_get_llm_model_default(mock_env_vars, mock_llm):
    ...
    mock_llm["google"].assert_called_once_with(
        model=TEST_MODEL, google_api_key=TEST_API_KEY
    )
...
def test_get_llm_model_openai(mock_llm):
    ...
    assert isinstance(llm, LLM)
    mock_llm["openai"].assert_called_once_with(
        model=TEST_MODEL, openai_api_key=TEST_API_KEY, base_url=TEST_API_BASE
    )
...
def test_get_llm_model_missing_provider():
    ...
    with pytest.raises(
        ValueError, match="LLM_PROVIDER not found in environment variables."
    ):
...
def test_get_llm_model_missing_model(mock_env_vars):
    ...
    with patch.dict(os.environ, {"LLM_MODEL": ""}):
        with pytest.raises(
            ValueError, match="LLM_MODEL not found in environment variables."
        ):
            get_llm_model()
